...
class CreateFileRunner
  ...
  _load-fence: (node) ~>
    | @content.length > 0  =>  @formatter.error 'found second content block for file to create, please provide only one', node.lines[0] + 1
  ...
  _load-text: (node) ~>
    ...
    | @file-path.length > 0  =>  @formatter.error "several file paths found: #{cyan @file-path} and #{cyan node.content}", @currently-loaded-node-line
    @file-path = node.content.trim!
    if @file-path.length is ''  then @formatter.error 'Empty file path found'
