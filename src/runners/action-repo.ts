...
// Lazy loader
export default memoize(function getActionRepo(): ActionRepo {
  return new ActionRepo()
})

// provides Actions to perform
class ActionRepo {
  // the actions built into Text-Runner
  private builtinActions: { [key: string]: Action }

  // user-defined actions
  private customActions: { [key: string]: Action }
  ...
  private loadBuiltinActions() {
    for (const filename of builtinActionFilePaths()) {
      this.builtinActions[getActionName(filename)] = require(filename).default
    }
  }
  ...
  private loadCustomActions() {
    ...
    for (const filename of customActionFilePaths()) {
      ...
      this.customActions[actionName] = require(filename)
    }
  }
}
