...
export default function verifyNpmGlobalCommand(args: ActionArgs) {
  ...
  const commandName = trimDollar(args.nodes.textInNodeOfType("fence", "code").trim())
...
function hasCommandName(commandName: string, exportedCommands: { [key: string]: string }): boolean {
