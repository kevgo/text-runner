 ...
 */
export async function getFileNames(config: configuration.Data): Promise<FullPath[]> {
 ...
 */
async function getFiles(config: configuration.Data): Promise<FullPath[]> {
  ...
  if (config.files === "") {
    ...
    return [new FullPath(config.files)]
 ...
 */
export async function markdownFilesInDir(dirName: string, sourceDir: string): Promise<FullPath[]> {
 ...
 */
export function removeExcludedFiles(fileList: FullPath[], excluded: string | string[]): FullPath[] {
