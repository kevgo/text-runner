...
import { AstNodeTags, AstNodeTypes } from "./standard-AST/ast-node"
...
test("TagMapper.tagForType()", function () {
  ...
  assert.equal(tagMapper.tagForType("foo_open" as AstNodeTypes), "foo", "unknown opening tag")
  assert.equal(tagMapper.tagForType("foo_open" as AstNodeTypes), "foo", "unknown closing tag")
  assert.equal(tagMapper.tagForType("foo" as AstNodeTypes), "foo", "unknown standalone tag")
...
test("TagMapper.typeForTag()", () => {
  ...
  assert.equal(tagMapper.typeForTag("foo" as AstNodeTags, {}), "foo_open", "unknown opening tag")
  assert.equal(tagMapper.typeForTag("/foo" as AstNodeTags, {}), "foo_close", "unknown closing tag")
})
