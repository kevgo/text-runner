...
suite("removeExcludedFiles", function () {
  test("single filename given", function () {
    const fileList = [new files.FullFile("one"), new files.FullFile("two")]
  ...
  test("array of filenames given", function () {
    const fileList = [new files.FullFile("one"), new files.FullFile("two"), new files.FullFile("three")]
  ...
  test("no excludes given", function () {
    const removedList = files.removeExcludedFiles([new files.FullFile("one")], [])
    expect(removedList).to.eql([{ value: "one" }])
  })

  test("relative folder name given", function () {
    const removedList = files.removeExcludedFiles(
      [new files.FullFile("one"), new files.FullFile("node_modules/zonk/broken.md")],
      "node_modules"
    )
    expect(removedList).to.eql([{ value: "one" }])
  })
