...
import * as cmdLineArgs from "./cmdline-args"

suite("parseCmdlineArgs()", function () {
  test("with unix <node> call", function () {
    const have = cmdLineArgs.parse([
  ...
  test("with <text-run> call", function () {
    const have = cmdLineArgs.parse(["/Users/kevlar/d/text-runner/bin/text-run", "run"])
    assert.equal(have.commandName, "run")
    assert.isUndefined(have.cmdLineConfig.files)
  })

  test("--online <file>", function () {
    const have = cmdLineArgs.parse(["--online", "documentation/actions/cd.md"])
    assert.equal(have.commandName, "run")
  ...
  test("<file>", function () {
    const have = cmdLineArgs.parse(["documentation/actions/cd.md"])
    assert.equal(have.commandName, "run")
    assert.equal(have.cmdLineConfig.files, "documentation/actions/cd.md")
  })

  test("(no args)", function () {
    const { commandName: command, cmdLineConfig: config } = cmdLineArgs.parse([])
    assert.equal(command, "run")
    assert.isUndefined(config.files)
  })

  test("--format=dot", function () {
    const { commandName, cmdLineConfig } = cmdLineArgs.parse(["--format=dot"])
    assert.equal(commandName, "run")
    assert.equal(cmdLineConfig.formatterName, "dot")
  })

  test("--workspace foo/bar", function () {
    const { commandName, cmdLineConfig } = cmdLineArgs.parse(["--workspace=foo/bar"])
    assert.equal(commandName, "run")
