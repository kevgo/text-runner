...
class MarkdownParser {
  ...
  _standardizeAst (ast, line: number = 0, result: AstNodeList = [], heading: ?Heading = undefined): AstNodeList {
    ...
    for (let node of ast) {
      ...
      } else if ([ 'code', 'fence', 'htmlblock', 'htmltag', 'link_open', 'text' ].indexOf(node.type) > -1) {
        result.push({line: nodeLine, type: `${modifiers.sort().join()}${node.type}`, content: (node.content || node.href)})
      }
